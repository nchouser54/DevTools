...
variable "image_repository" {
  type        = string
  description = "Container image repository (e.g. 012345678901.dkr.ecr.us-gov-west-1.amazonaws.com/bedrock-chatbot-rag)."
  default     = "replace-me/eks-bedrock-chatbot-rag"
}
...
locals {
  template_name = length(trimspace(var.workspace_name)) > 0 ? trimspace(var.workspace_name) : try(data.coder_workspace.me.name, "eks-bedrock-chatbot-rag")
  template_tags = ["eks", "bedrock", "azure-openai", "rag", "chatbot", "knowledge-bases", "opensearch"]
...
output "template_summary" {
  ...
  description = "RAG workspace metadata."
}

output "chatbot_service_name" {
  value       = kubernetes_service.chatbot.metadata[0].name
  description = "Kubernetes Service name for accessing the RAG chatbot."
}

output "chatbot_namespace" {
  value       = kubernetes_namespace.chatbot.metadata[0].name
  description = "Kubernetes namespace where RAG chatbot is deployed."
}
